...
class DroneCore:
    ...
    def _generate_noise_file(self, sample_rate):
        ...
        try:
            ...
            print(f"[ГЛУШ] Файл шума создан: {self.noise_file_path}, размер {os.path.getsize(self.noise_file_path)} байт")
        except Exception as e:
            print(f"[ГЛУШ] Ошибка генерации шума: {e}")
    ...
    def start_jamming(self, freq):
        ...
        try:
            ...
            if self.jamming_process.poll() is not None:
                ...
                print(f"[ГЛУШ] Процесс завершился сразу. stderr: {stderr.decode()}")
                self.jamming_process = None
                return False
            self.jamming_active = True
            self.jamming_start_time = time.time()
            print(f"[ГЛУШ] Передача начата на {freq/1e6:.3f} МГц, усиление {self.jamming_power} дБ")
            return True
        except Exception as e:
            print(f"[ГЛУШ] Ошибка запуска глушения: {e}")
            self.jamming_process = None
            return False
    ...
    def stop_jamming(self):
        ...
        if self.jamming_process is not None:
            ...
            self.jamming_process = None
            self.jamming_active = False
            print("[ГЛУШ] Передача остановлена.")
    ...
    def send_email_alert(self, freq, rssi):
        if not self.email_enabled:
            return
        try:
            msg = MIMEText(f"Обнаружен сигнал на частоте {freq/1e6:.3f} МГц\nRSSI: {rssi:.1f} дБ\nВремя: {datetime.now()}")
            msg['Subject'] = '🚁 Drone Detector: Тревога!'
    ...
    def scan_once(self):
        results = {}
        # Если адаптивный порог включён и нужна калибровка – собираем шум без тревог
        if self.adaptive_threshold_enabled and self.calibration_needed:
            ...
            if self.calibration_counter >= self.calibration_samples:
                ...
                self.calibration_counter = 0
                print("[АДАПТ] Калибровка завершена, адаптивный порог активен.")
            return results

        # Обычное сканирование (после калибровки или без адаптивного порога)
        for freq in self.frequencies:
            rssi = self.get_rssi(freq)
            self.history[freq].append(rssi)
            ...
            results[freq] = {
                ...
                "movement": mov,
                "alert": alert,
                "confidence": "ВЫСОКАЯ" if (sig and sig["is_drone_like"]) else "СРЕДНЯЯ",
            }
        return results
...
class DroneDetectorGUI:
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Drone Detector v0.5 (Русская версия)")
    ...
    def _apply_settings_to_core(self):
        ...
        # Сброс калибровки, если включён адаптивный порог
        if self.core.adaptive_threshold_enabled:
            ...
            self._log_append("Адаптивный порог включён – начинается калибровка...", tag="calibration")
        else:
            self.core.calibration_needed = False

        self._rebuild_plots(freqs, thrs)
        self._rebuild_tree(freqs)
        self._log_append("Настройки применены к ядру.", tag="info")
    ...
    def _build_ui(self):
        ...
        tk.Button(top, text="Выйти из слежения", command=self._exit_tracking,
        ...
        nb.add(monitor_frame, text="  Мониторинг  ")
    ...
    def _init_settings_vars(self):
        params = [
            ("std_window", "Окно для STD (точек)", "20"),
            ("std_threshold", "Порог STD (dB)", "3.0"),
            ("autocorr_threshold", "Макс. автокорреляция", "0.6"),
            ("crossings_threshold", "Мин. пересечений нуля", "5"),
            ("trend_sensitivity", "Чувств. движения (dB/шаг)", "0.3"),
    ...
    def _build_monitor_tab(self, parent):
        ...
        tk.Label(left, text="Состояние частот", font=("Arial", 10, "bold"),
        ...
        self._tree.heading("freq", text="МГц")
        self._tree.heading("rssi", text="RSSI, dB")
        self._tree.heading("threshold", text="Порог")
        ...
        tk.Label(left, text="Лог событий", font=("Arial", 10, "bold"),
    ...
    def _build_plots(self, parent):
        ...
        for i, freq in enumerate(DEFAULT_FREQUENCIES):
            ...
            ax.set_title(f"{freq/1e6:.1f} МГц", fontsize=8, loc="left", pad=2)
            ax.set_ylim(-80, 0)
    ...
    # ---------- Вкладка "Настройки" с прокруткой ----------
    def _build_settings_tab(self, parent):
        ...
        headers = ["Частота (Гц)", "Вкл", "Порог (dB)", "BW, МГц (2-20)", ""]
        ...
        analysis_descs = {
            "std_window": "Чем больше, тем стабильнее оценка разброса",
            "std_threshold": "Минимальное стандартное отклонение для сигнала",
            "autocorr_threshold": "Чем выше, тем более периодичный сигнал",
            "crossings_threshold": "Показывает изменчивость формы сигнала",
            "trend_sensitivity": "Порог тренда для определения приближения",
            "trend_window": "Длина истории для оценки движения",
        }
    ...
    def _add_frequency(self):
        ...
        if bw < 2.0 or bw > 20.0:
            messagebox.showerror("Ошибка", "Полоса пропускания должна быть от 2 до 20 МГц")
            return
        for fvar in self._freq_vars:
            if abs(float(fvar.get()) - f) < 1e-3:
                messagebox.showerror("Ошибка", f"Частота {f/1e6:.3f} МГц уже существует")
                return
    ...
    def _remove_frequency(self, idx):
        if len(self._freq_vars) <= 1:
            messagebox.showwarning("Внимание", "Нельзя удалить единственную частоту")
            return
    ...
    def _apply_settings(self, silent=False):
        if self.running and not silent:
            messagebox.showwarning("Внимание", "Остановите сканирование перед изменением настроек.")
            return
        ...
        for i, fvar in enumerate(self._freq_vars):
            ...
            try:
                ...
                if not silent:
                    messagebox.showerror("Ошибка", f"Строка {i+1}: неверный формат")
                return
            if bw_mhz < 2.0 or bw_mhz > 20.0:
                if not silent:
                    messagebox.showerror("Ошибка", f"Строка {i+1}: полоса должна быть от 2 до 20 МГц")
                return
            freqs.append(f)
            thrs[f] = t
            bws[f] = bw_mhz * 1e6
        if not freqs:
            if not silent:
                messagebox.showerror("Ошибка", "Выберите хотя бы одну частоту")
            return
        ...
        try:
            ...
            if not silent:
                messagebox.showerror("Ошибка", "Проверьте параметры анализа (должны быть числа)")
            return
        ...
        self._rebuild_plots(freqs, thrs)
        self._rebuild_tree(freqs)
        self._log_append("Настройки применены." if not silent else "Настройки загружены.", tag="info")
        self._save_settings()

    # ---------- Перестройка графиков ----------
    def _rebuild_plots(self, freqs, thrs):
        ...
        for i, freq in enumerate(freqs):
            ax = self._fig.add_subplot(gs[i])
            ax.set_title(f"{freq/1e6:.3f} МГц", fontsize=8, loc="left", pad=2)
            ax.set_ylim(-80, 0)
            ax.set_xlim(0, PLOT_WINDOW)
            ax.set_ylabel("dB", fontsize=7)
            ax.tick_params(labelsize=7)
    ...
    def _build_stats_tab(self, parent):
        ...
        tk.Label(wrapper, text="Статистика обнаружений",
        ...
        tk.Button(wrapper, text="Обновить",
    ...
    def _update_stats(self):
        ...
        if self.core is None:
            self._stats_text.insert(tk.END, "Детектор не запущен.\n")
        else:
            lines = [f"{'Частота (МГц)':<20} {'Обнаружений':>14}\n", "-"*36+"\n"]
    ...
    def _on_start(self):
        ...
        if not DroneCore.check_hackrf():
            messagebox.showerror("HackRF не найден", "Подключите HackRF и попробуйте снова.")
            return
        ...
        self._log_append("Сканирование запущено.", tag="info")
    ...
    def _on_stop(self):
        ...
        self._mode_var.set("")
        self._log_append("Сканирование остановлено.", tag="info")
        self._update_stats()
    ...
    def _exit_tracking(self):
        if self.core and self.core.tracking_mode:
            ...
            self.core.stop_jamming()
            self._mode_var.set("")
            self._log_append("Выход из режима слежения.", tag="info")
    ...
    def _apply_scan_results(self, data):
        ...
        if core.adaptive_threshold_enabled and core.calibration_needed:
            self._status_var.set(f"Калибровка... ({core.calibration_counter}/{core.calibration_samples})")
            for freq in core.frequencies:
                try:
                    self._tree.item(str(freq), values=(
                        f"{freq/1e6:.3f}", "—", "—", "КАЛИБР"
                    ), tags=("ok",))
                except tk.TclError:
                    pass
            try:
        ...
        for i, freq in enumerate(core.frequencies):
            ...
            status = "ТРЕВОГА" if alert else ("OK" if rssi > thr else "—")
            ...
            if alert:
                ...
                self._log_append(
                    f"[{datetime.now().strftime('%H:%M:%S')}] СИГНАЛ {freq/1e6:.1f} МГц | "
    ...
    def _apply_tracking_result(self, data):
        ...
        if jamming:
            self._mode_var.set(f"ГЛУШЕНИЕ {freq/1e6:.1f} МГц")
            self._status_var.set(f"RSSI: {rssi:.1f} dB (глушим)")
            try:
                self._tree.item(str(freq), values=(
            ...
            if not hasattr(self, '_last_jam_log') or time.time() - self._last_jam_log > 5:
                self._log_append(f"[{datetime.now().strftime('%H:%M:%S')}] Глушение на {freq/1e6:.1f} МГц (RSSI={rssi:.1f} dB)", tag="jamming")
                self._last_jam_log = time.time()
        else:
            self._mode_var.set(f"СЛЕЖЕНИЕ {freq/1e6:.1f} МГц")
        ...
        if lost:
            ...
            self._mode_var.set("")
            self._log_append(
                f"[{datetime.now().strftime('%H:%M:%S')}] Сигнал {freq/1e6:.1f} МГц ослаб — выход из слежения",
                tag="track"
            )
